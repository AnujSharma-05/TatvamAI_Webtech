...
export default function SignInPage() {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post('/users/login', { email: contact, password });
      const tokens = handleLoginResponse(res);
      if (tokens) {
        toast.success('Signed in successfully!');
        window.dispatchEvent(new Event('storage'));
      ...
      toast.error(err.response?.status === 404 ? 'This email is not registered.' : message);
    } finally {
      setLoading(false);
    }
  };
      ...
      >
        ...
        >
          <div className="text-center mb-8">
            <h1 className="text-4xl font-extrabold mb-2" style={{color: COLORS.nyanza}}>Welcome Back</h1>
          ...
          <div className="flex p-1 rounded-xl mb-8" style={{background: `${COLORS.midnightGreen}40`}}>
            <button
              ...
              className={`w-1/2 py-2 rounded-lg font-semibold transition-colors duration-300 ${authMethod === 'phone' ? 'text-midnightGreen' : 'text-white'}`}
              style={{background: authMethod === 'phone' ? COLORS.teaGreen : 'transparent'}}
            >
              Phone
            </button>
            <button
              onClick={() => setAuthMethod('email')}
              className={`w-1/2 py-2 rounded-lg font-semibold transition-colors duration-300 ${authMethod === 'email' ? 'text-midnightGreen' : 'text-white'}`}
              style={{background: authMethod === 'email' ? COLORS.teaGreen : 'transparent'}}
            >
              Email
            </button>
          </div>
          ...
          {authMethod === 'email' ? (
              <form onSubmit={handleEmailLogin} className="space-y-6">
                <div>
                  <StyledInput type="email" value={contact} onChange={(e) => setContact(e.target.value)} placeholder="Enter your email" required />
                </div>
                <div>
                  <div className="relative">
                    <StyledInput type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter your password" required />
                    <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute inset-y-0 right-0 pr-4 flex items-center">
                      {showPassword ? <EyeOff className="h-5 w-5" style={{color: COLORS.cadetGray}} /> : <Eye className="h-5 w-5" style={{color: COLORS.cadetGray}} />}
                    </button>
                  </div>
                </div>
                <button type="submit" disabled={loading} className="w-full py-3 rounded-xl font-semibold transition-all" style={{background: COLORS.teaGreen, color: COLORS.midnightGreen, opacity: loading ? 0.7 : 1}}>
              ...
              <form onSubmit={handleSendOtp} className="space-y-6">
                <div>
                  <StyledInput type="tel" value={contact} onChange={(e) => setContact(e.target.value)} placeholder="Enter 10-digit number" required maxLength={10} />
                </div>
                <button type="submit" disabled={loading} className="w-full py-3 rounded-xl font-semibold transition-all" style={{background: COLORS.teaGreen, color: COLORS.midnightGreen, opacity: loading ? 0.7 : 1}}>
              ...
              <form onSubmit={handleVerifyOtp} className="space-y-6">
                <div className="text-center">
                   <StyledInput type="text" value={otp} onChange={(e) => setOtp(e.target.value)} placeholder="Enter OTP" required maxLength={6} className="text-center text-2xl tracking-[0.5em]" />
                </div>
                <button type="submit" disabled={loading} className="w-full py-3 rounded-xl font-semibold transition-all" style={{background: COLORS.teaGreen, color: COLORS.midnightGreen, opacity: loading ? 0.7 : 1}}>
                  {loading ? 'Verifying...' : 'Verify & Sign In'}
                </button>
                <button type="button" onClick={() => setIsOtpSent(false)} className="w-full text-sm text-center" style={{color: COLORS.cadetGray}}>
                  Change phone number
                </button>
              </form>
          )}
        ...
        <p className="text-center mt-8" style={{color: COLORS.cadetGray}}>
          ...
          <Link to="/auth/signup" className="font-semibold hover:underline" style={{color: COLORS.teaGreen}}>
            Sign up
          </Link>
        </p>
